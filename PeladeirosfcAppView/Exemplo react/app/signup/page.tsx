  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const userData: Partial<User> = {
        ...
        passwordHash:formData.senha, // Simple hash for now, should be done on backend
        phoneNumber: "",
        ...
        dataNascimento: formData.dataNascimento ? new Date(formData.dataNascimento).toISOString() : "",
        genero: formData.genero,
        fotoUrl: fotoPreview || "",
        cidade: formData.cidade,
        bairro: formData.bairro,
        cep: formData.cep,
        altura: formData.altura ? parseInt(formData.altura) : 0,
        peso: formData.peso ? parseInt(formData.peso) : 0,
        tamanhoPe: formData.tamanhoPe ? parseInt(formData.tamanhoPe) : 0,
        peDominante: formData.peDominante,
      ...
      setFotoPreview(null)
      alert("Conta criada com sucesso! Você será redirecionado para login.")
    } catch (err) {
        ...
        <div className="flex flex-1 justify-center px-5 sm:px-6 pb-8">
          <div className="w-full max-w-lg space-y-5 sm:space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
              ...
              {success && (
                <div className="bg-green-100 border border-green-400 text-green-700 px-3 sm:px-4 py-3 rounded text-sm">
                  ✓ Vá no seu email {formData.email} para confirmar sua conta.
                </div>
              )}
              {/* Dados Pessoais */}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 sm:gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Select value={formData.peDominante} onValueChange={(value) => handleSelectChange('peDominante', value)}>
                      <SelectTrigger className="h-10 sm:h-11 border-border bg-card/80 text-foreground text-sm">
                        <SelectValue placeholder="Selecione" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={formData.posicao} onValueChange={(value) => handleSelectChange('posicao', value)}>
                      <SelectTrigger className="h-10 sm:h-11 border-border bg-card/80 text-foreground text-sm">
                        <SelectValue placeholder="Selecione" />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <Button 
                type="submit" 
                disabled={loading}
                className="h-11 sm:h-12 w-full bg-primary text-primary-foreground hover:bg-primary/90 disabled:bg-gray-400 text-sm sm:text-base font-semibold"
              >
